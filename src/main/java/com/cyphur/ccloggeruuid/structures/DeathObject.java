...
public class DeathObject {
   ...
   public DeathObject(PlayerDeathEvent event, String template) {
      this._player = event.getEntity().getPlayer();
      this._killer = this._player.getKiller();
      if (this._killer != null) {
         this._message = this._player.getName() + " was killed by " + this._killer.getName();
      } else {
         this._message = this._player.getName() + " was killed.";
      }
